...
d = vpa(d(:),77); % Make sure d is a column vector.
...
if all(d<bits32),
    ...
    dnext=d;
    for i=1:7
        ds(i)=vpa(floor(dnext./(bits32^(8-i))),77);
        dnext=vpa(rem(dnext,bits32^(8-i)),77);
    end
    ds(8)=vpa(rem(dnext,bits32),77);
