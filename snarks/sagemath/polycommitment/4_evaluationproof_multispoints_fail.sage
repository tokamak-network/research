...
_a = FF(30) #toxic, it should be disappear after created, no one knows.
...
Fx = P([FF.random_element() for i in range(d+1)]).polynomial() ## FF based
...
px = [FF[x]((-B[i], 1)) for i in range(t)]
print("px : {}".format(px))
...
def weil(point1, point2, order):
    val = EC.isomorphism_to(ECExt)(point1).weil_pairing(point2, order)
    return val

def tate(point1, point2):
    val = EC.isomorphism_to(ECExt)(point1).tate_pairing(point2, 72, 2)
    return val

def weil2(point1, point2):
    val = ECExt.isomorphism_to(EC)(point1).weil_pairing(point2, 72)
    return val

LHS = tate(Cf1 - Cr1, g2)
RHS = tate(Cq1, Cp2)
# LHS = weil2(g2, Cf1 - Cr1)
# RHS = weil2(Cp2, Cq1)
